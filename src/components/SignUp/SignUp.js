...
const SignUp = () => {
    const [user, setUser] = useState({ name: "", email: "", password: "", confirmPassword: "", isSignUp: false, success: false, errorCode: "", errorMessage: "" });
    ...
    const handleSubmit = (e) => {
        ...
        if (!error.password && !error.confirmPassword) {
            createUserWithEmailAndPassword(user.name, user.email, user.password)
                .then(res => {
                    const newSignUpInfo = { ...user };
                    newSignUpInfo.isSignUp = res.isSignUp;
                    newSignUpInfo.success = res.success;
                    newSignUpInfo.errorCode = res.errorCode;
                    newSignUpInfo.errorMessage = res.errorMessage;
                    setUser(newSignUpInfo);
                    if (newSignUpInfo.isSignUp)
                        history.push("/login");
                });
        }
    ...
    return (
        <div className="login">
            ...
            <div className="login-form">
                ...
                {
                    user.isSignUp ? <p style={{ color: "green" }}>Mr.{user.email} Sign Up successfully</p> : <p style={{ color: "red" }}>{user.errorMessage}</p>
                }
                <br /><br /><br /><br /><br /><br />
            </div>
        </div>
